...
6. 普通交互模式会询问是否校准完整邮件转发点击区域；首次部署建议在 `--no-forward` 下选择 `y` 安全验证。
7. 输入本次运行时间（秒）；留空或输入 `0` 表示持续运行。
8. 将鼠标放到第一位候选人卡片上。
9. 第一位候选人详情打开后，按提示框选焦点恢复空白区域、五个转发点击区域和 OCR 详情正文区域。
...
| `--auto` | 跳过交互输入和全部转发区域校准，不导航弹窗，使用命令行参数及默认区域 |

不提供关键词时，OCR 和自动转发均禁用，只执行候选人浏览。
有关键词的交互模式会在启动时询问是否校准完整转发点击区域，随后询问本次运行秒数；无关键词时不询问转发校准。使用 `--auto` 时可通过 `--duration-seconds 3600` 设置自动停止时间；非法值会阻止程序启动。
...
1. 新电脑、新分辨率或页面布局变化后，先以有关键词的普通交互方式使用 `--no-forward`，验证 OCR 和全部转发点击区域框选。
