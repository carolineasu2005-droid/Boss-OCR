...
6. 输入本次运行时间（秒）；留空或输入 `0` 表示持续运行。
7. 将鼠标放到第一位候选人卡片上。
8. 第一位候选人详情打开后，在半透明界面中框选详情正文区域。
9. 观察日志和实际页面，确认 OCR 命中正确后，再考虑使用 `正式运行.bat`。
...
同一条规则内支持 `and` 和 `or`，其中 `and` 优先于 `or`；分号分隔的多条规则只要任一条命中即可。程序不支持括号，也不做编辑距离、相似字替换、拼音匹配、同义词推断、语义匹配或大模型判断。
...
首次命中规则后，程序不会立即转发。它保持当前滚动位置，等待约 `0.7` 秒，对同一区域再次截图和 OCR。两次都命中同一整条规则，才产生 `confirmed_match=True`。
...
```powershell
BossOCR.exe --keywords '"Python" and "AE"; "短剧" or "带货"' --email "backup@example.com" --auto
```
...
交互模式会在启动时询问运行秒数。使用 `--auto` 时可通过 `--duration-seconds 3600` 设置自动停止时间；非法值会阻止程序启动。
...
`and` 要求同组关键词全部命中，`or` 要求任一关键词命中；混用时 `and` 优先。不支持括号。旧配置 `Python;短剧` 必须手动迁移为 `"Python";"短剧"`，程序不会静默改写错误格式。
