...
- 激活 Chrome、确认 frontmost，并只读查询 active tab URL/title；
- 对 active tab 执行保守 BOSS 页面身份白名单诊断；
...
5. 激活 Chrome 并确认 frontmost；
6. 只读查询 active tab URL/title 并执行 BOSS 页面身份白名单判断；
7. 输出结构化结果并退出。

权限 baseline 为 `unknown` 时仍会继续上述只读诊断，以提供更完整的本地结果；
这不表示权限充分，更不会使 macOS 进入业务 ready。
...
5. 确认 Chrome 最多只打开 `about:blank`，终端输出至少包含：
   ...
   如果 Chrome 未安装、路径异常、不可执行、启动失败、无法置前或标签页查询失败，
   可以出现相应的明确 fail-closed 错误码。即使页面命中白名单，也只会返回
   `MACOS_PAGE_ALLOWED_NOT_BUSINESS_READY`，且 `ready` 仍为 `False`。
...
本清单通过只能证明 Chrome 安全启动、权限 baseline、窗口置前与页面身份只读诊断边界正常。
在 Retina 坐标、校准、OCR 与真实页面业务安全验证完成前，macOS 不得进入 BossOCR 业务模式。
