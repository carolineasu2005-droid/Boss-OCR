# BossOCR Windows Stable v1.1

BossOCR Windows Stable v1.1 是面向 Windows 10/11 x64 + Microsoft Edge 的稳定发布版。本版将 Issue #1 的最小可行改动、OCR 本地识别以及后续 Next-1 至 Next-3 稳定性工作整合为一个可直接解压运行的 Windows 发布包。
...
- P0 hotfix：转发流程结束后统一执行两次焦点恢复点击，降低转发失败后页面焦点未恢复、右方向键无效、继而重复处理同一份简历的风险。该修复不检测转发成功/失败页面，也不改变右方向键切换下一位候选人的既有流程。
...
- 验证环境：另一台 Windows 电脑 + Microsoft Edge + BOSS 页面。
- 验证内容：完整校准流程、OCR/关键词流程，以及 `--no-forward` 下即使关键词命中也不执行真实转发。
- 校准顺序已验证为：焦点恢复 → 转发入口 → 邮件 Tab → 邮箱输入框 → 最近联系 → 转发按钮；框选最终转发按钮时未发送邮件。

> 这是在另一台 Windows 电脑上完成的人工实机 GUI 验证，不是当前构建环境中执行的自动化 GUI 测试。

### 真实转发最小验证

**人工实机验证通过。**

- 使用测试邮箱，仅验证一位候选人，并由人工全程监控页面和鼠标操作。
- 真实邮件转发可以完成。
- 转发结束后焦点恢复生效，程序可以继续进入下一位候选人。
- 该结果为单位候选人的最小受控验证，不是批量或无人值守验证。

## 发布产物

- 应用：`dist\BossOCR\BossOCR.exe`
- 主发布附件：`release\BossOCR-Windows-x64.zip`
- ZIP SHA-256：`795D2D8DBC109E0D5ACA6997562942C188C722330AD06B33BCB4A48A972E929E`

`dist/`、`build/`、`release/`、exe、ZIP、虚拟环境和缓存均不提交到 Git，仅将 ZIP 作为 GitHub Release 附件。

## 已知限制

- 本版仍依赖当前 BOSS 页面结构和现有流程，页面布局或交互大幅改变后需要重新验证。
- 当前为 Windows + Microsoft Edge 版本，不包含 macOS Chrome 支持。
- 校准区域仍是仅当前运行期有效的绝对屏幕坐标，不持久化。
- 校准后移动或缩放 Edge 窗口、改变分辨率或浏览器缩放会使区域失效，必须重启并重新校准。
- `--auto` 不询问、不弹框选层、不执行校准导航，始终使用默认区域。
- OCR 仍可能受字体清晰度、动画、遮挡、加载状态和识别误差影响，存在小概率漏看。
- 框选第一版仅支持主显示器。
- 程序不识别页面状态，也不检测转发成功或失败页面；用户必须在界面与提示不符时按 Esc 停止或取消。
